...
class ClientCore:
    ...
    def can_retry_request(self, prepared: PreparedRequest) -> bool:
        return safe_to_retry(prepared.method, prepared.headers.get(self.idempotency.header.lower()), self.retry_policy)
...
class SyncClient:
    ...
    def request_raw(self, method: str, path: str, _stream: bool = False, **options: Any) -> httpx.Response:
        ...
        while True:
            ...
            if not decision.retry or not self._core.can_retry_request(prepared):
    ...
    def request_page(self, request: PageRequest) -> Any:
        ...
        result = self.request(
            ...
            query_styles=request.query_styles,
